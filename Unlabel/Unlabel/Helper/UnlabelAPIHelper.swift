...
class UnlabelAPIHelper{
  ...
  func getUccAvailability(_ ucc: String, success:@escaping ( _ json:JSON,_ statusCode:Int)->(),failed:@escaping (_ error:NSError)->()){
    let requestURL:String?
    requestURL = v4BaseUrl + "api_v2/influencer_ucc_handle/"
    if let requestURLObj = requestURL{
      Alamofire.request(requestURLObj, method: .get, parameters: ["ucc_handle":ucc]).responseJSON { response in
        switch response.result {
        case .success(let data):
          let json = JSON(data)
          success(json,(response.response?.statusCode)!)
